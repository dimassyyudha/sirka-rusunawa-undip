...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-900">Edit Penghuni</h2>
                <p class="text-sm text-slate-500 mt-1">Perbarui data {{ $penghuni->user->name ?? '-' }}.</p>
            </div>
        ...
        <form action="{{ route('admin.penghuni.update', $penghuni) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="nim" value="{{ old('nim', $penghuni->nim) }}" maxlength="14" required
                        class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 focus:ring-4 focus:ring-violet-200 focus:border-violet-500 focus:outline-none">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="number_phone"
                        value="{{ old('number_phone', $penghuni->user->number_phone) }}"
                        class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 focus:ring-4 focus:ring-violet-200 focus:border-violet-500 focus:outline-none">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="room_id"
                        ...
                        @foreach ($rooms as $room)
                            ...
                            <option value="{{ $room->id }}" @selected(old('room_id', $penghuni->room_id) == $room->id)>
                                {{ $room->kode_kamar }}
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat" rows="3"
                        class="w-full rounded-2xl border border-slate-200 px-4 py-3 resize-none text-sm font-semibold text-slate-700 focus:ring-4 focus:ring-violet-200 focus:border-violet-500 focus:outline-none">{{ old('alamat', $penghuni->alamat) }}</textarea>
                </div>
