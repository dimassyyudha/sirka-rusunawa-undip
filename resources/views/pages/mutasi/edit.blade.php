        ...
        <div class="card-body">
            <form action="{{ route('mutasi.update', $mutasi->mutasi_id) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <label>Tanggal</label>
                        <input type="date" name="tanggal" class="form-control" value="{{ $mutasi->tanggal->format('Y-m-d') }}" required>
                    </div>

                    <div class="col-12 mb-3">
                        <label>Jenis Mutasi</label>
                        <select name="jenis_mutasi" class="form-select" required>
                            @php $opsi = ['Pindah Lokasi', 'Alih Tanggung Jawab', 'Rusak Berat', 'Hibah', 'Lainnya']; @endphp
                            @foreach($opsi as $op)
                                <option value="{{ $op }}" {{ $mutasi->jenis_mutasi == $op ? 'selected' : '' }}>{{ $op }}</option>
                            @endforeach
                        </select>
                    </div>
